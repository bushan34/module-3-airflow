...
SQL_CONTEXT = {
    'LOAD_VIEW_PAYMENT_ONE_YEAR': """
          create or replace view nnaranov.view_pro_payment_{{ execution_date.year }} as (
            with staging as (
              with derived_columns as (
                select
                  ...
                  'payment-DataLake-project'::varchar as RECORD_SOURCE
                from nnaranov.pro_ods_payment
    ...
    'HUBS': {
            'HUB_USER':  """
                    with row_rank_1 as (
                         select * from (
                                 ...
                                 from nnaranov.view_pro_payment_{{ execution_date.year }}         
                         ) as h where row_num = 1
                    ),  
                    records_to_insert as (
                         select a.USER_PK, a.USER_KEY, a.LOAD_DATE, a.RECORD_SOURCE
                         from row_rank_1 as a
                         left join nnaranov.pro_dds_hub_user as d
                              on a.USER_PK = d.USER_PK
                         where d.USER_PK is NULL
                    )
                    insert into nnaranov.pro_dds_hub_user (USER_PK, USER_KEY, LOAD_DATE, RECORD_SOURCE)
                    (
            ...
            'HUB_ACCOUNT':  """
                    ...
                    records_to_insert as (
                         ...
                         from row_rank_1 as a
                         left join nnaranov.pro_dds_hub_account as d
                              on a.ACCOUNT_PK = d.ACCOUNT_PK
                         where d.ACCOUNT_PK is NULL
                    )
                    insert into nnaranov.pro_dds_hub_account (ACCOUNT_PK, ACCOUNT_KEY, LOAD_DATE, RECORD_SOURCE)
                    (
            ...
            'HUB_BILLING_PERIOD':  """
                    ...
                    records_to_insert as (
                        ...
                        from row_rank_1 as a
                        left join nnaranov.pro_dds_hub_billing_period as d
                             on a.BILLING_PERIOD_PK = d.BILLING_PERIOD_PK
                        where d.BILLING_PERIOD_PK is NULL
                    )
                    insert into nnaranov.pro_dds_hub_billing_period (BILLING_PERIOD_PK, BILLING_PERIOD_KEY, LOAD_DATE, RECORD_SOURCE)
                    (
            ...
            'HUB_PAY_DOC':  """
                  ...
                  records_to_insert as (
                      ...
                      from row_rank_1 as a
                      left join nnaranov.pro_dds_hub_pay_doc as d
                      on a.PAY_DOC_PK = d.PAY_DOC_PK
                      where d.PAY_DOC_PK is NULL
                  )
                  insert into nnaranov.pro_dds_hub_pay_doc (PAY_DOC_PK, PAY_DOC_TYPE_KEY, PAY_DOC_NUM_KEY, LOAD_DATE, RECORD_SOURCE)
                  (
    ...
    'LINKS': {
            'LINK_USER_ACCOUNT_BILLING_PAY':  """
                      with records_to_insert as (
                          ...
                          from nnaranov.view_pro_payment_{{ execution_date.year }} as stg 
                          left join nnaranov.pro_dds_link_user_account_billing_pay as tgt
                          on stg.USER_ACCOUNT_BILLING_PAY_PK = tgt.USER_ACCOUNT_BILLING_PAY_PK
                          where tgt.USER_ACCOUNT_BILLING_PAY_PK is null		
                      )
                      insert into nnaranov.pro_dds_link_user_account_billing_pay (
                          USER_ACCOUNT_BILLING_PAY_PK,
    ...
    'SATELLITES': {
            'SAT_USER_DETAILS':  """
                    ...
                    update_records as (
                        ...
                        from nnaranov.pro_dds_sat_user_details as a
                    ...
                    insert into nnaranov.pro_dds_sat_user_details (
                        USER_PK, USER_HASHDIFF, 
                        phone, 
                        EFFECTIVE_FROM, 
                        LOAD_DATE, RECORD_SOURCE)
            ...
            'SAT_PAY_DETAILS':  """
                    ...
                    update_records as (
                        select 
                            ...
                            a.EFFECTIVE_FROM, 
                            a.LOAD_DATE, a.RECORD_SOURCE
                        from nnaranov.pro_dds_sat_pay_details as a
                        join source_data as b
                    ...
                    insert into nnaranov.pro_dds_sat_pay_details (
                        USER_ACCOUNT_BILLING_PAY_PK, PAY_DOC_HASHDIFF, 
                        pay_date, sum, 
                        EFFECTIVE_FROM, 
                        LOAD_DATE, RECORD_SOURCE)
...
def get_phase_context(task_phase):
    ...
    for task in SQL_CONTEXT[task_phase]:
        ...
        tasks.append(PostgresOperator(
            task_id='pro_dds_{}_{}'.format(task_phase, task),
            dag=dag,
            sql=query
        ))
    return tasks
